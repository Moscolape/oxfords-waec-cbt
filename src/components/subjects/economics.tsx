...
const Economics = () => {
  ...
  const [answers, setAnswers] = useState<{ [key: string]: string | string[] }>(
    () => JSON.parse(localStorage.getItem("economicsQuizAnswers") || "{}")
  );
  ...
  const [hasSubmitted, setHasSubmitted] = useState(() =>
    JSON.parse(localStorage.getItem("economicsHasSubmitted") || "false")
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true);
    localStorage.setItem("economicsHasSubmitted", "true");
    ...
    try {
      ...
      localStorage.removeItem("economicsQuizAnswers");
      localStorage.removeItem("economicsHasSubmitted");
      localStorage.removeItem("economicsQuizStartTime");
    } catch (error) {
      console.error("Submission error:", error);
      setHasSubmitted(false);
      localStorage.setItem("economicsHasSubmitted", "false");
  ...
  useEffect(() => {
    const storedStartTime = localStorage.getItem("economicsQuizStartTime");
    ...
    if (storedStartTime) {
      ...
      startTime = Date.now();
      localStorage.setItem("economicsQuizStartTime", startTime.toString());
    }
  ...
  useEffect(() => {
    localStorage.setItem("economicsQuizAnswers", JSON.stringify(answers));
  }, [answers]);
    ...
    <DashboardWrapper>
      <div className="w-full max-w-6xl mx-auto font-Inter p-5 pt-25">
        <div className="flex justify-between items-center mb-5">
          <h1 className="text-2xl font-bold">Economics Test</h1>
